...
void ftl_read(UINT32 const lba, UINT32 const total_sectors)
{
	...
	if (lba >= READ_LBN && lba < (READ_LBN + READ_AREA_SIZE)) {
		uart_printf("lba in READ_AREA : %d reached\n", lba);
	}
...
void ftl_write(UINT32 const lba, UINT32 const total_sectors)
{
	...
	while (remain_sectors != 0)
	{
		...
		else if (lba == PARAMETER_LBN || lba == READ_LBN)
		{
			if (lba == READ_LBN) {
				uart_printf("write on READ_LBN : %d\n", lba);
			}
			else {
...
static void sanity_check(void)
{
	UINT32 dram_requirement = RD_BUF_BYTES + WR_BUF_BYTES + COPY_BUF_BYTES + FTL_BUF_BYTES
		+ HIL_BUF_BYTES + TEMP_BUF_BYTES + SCAN_LIST_BYTES + PAGE_MAP_BYTES  + INPUT_BUF_BYTES + OUTPUT_BUF_BYTES;
